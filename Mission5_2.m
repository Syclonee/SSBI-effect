...
for x=1:length(N_iterations)
    for k=1:length(CSPRdB)
        ...
        N_=length(It1);
        [Snt] = iterative_SSBI(It1,st,Pcarrier,N_iterations(x));%filtering
    ...
    % plot
    figure(1)
    semilogy(CSPRdB,BER,'linewidth',1.5),grid on,hold on;
    name{x}=append(string(N_iterations(x)),' iterations');
...
for x=1:length(N_iterations)
    ...
    semilogy(CSPRdB,BER,'linewidth',1.5),grid on,hold on;
    name{x}=append(string(N_iterations(x)),' iterations');
    title('16-QAM BER BER');legend(name{:});hold on;
    xlabel('CSPR(dB)');ylabel('Bit Error Rate');
end
